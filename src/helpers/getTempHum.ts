...
export const getTempHum = async ({ boardId, data, endDate, filter, startDate, numberPage, setPage, isGraphicFetching, isFormButtons } : GetDataProps) => {
  switch(filter) {
    case "Mostrar todo": {
      try {
        ...
        !isFormButtons && Swal.fire("Error", "No se encontró ningún registro en la tabla Temperature/Humidity", "error");
        return [];
      }
      break;
    }
