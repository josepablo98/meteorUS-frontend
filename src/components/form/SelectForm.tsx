    ...
    <div className="row">
      ...
      </div>
      {
        (register !== "-" && register !== "Registros de placas") && (
          <div className="col-md-4">
            <label>Filtros:</label>
            <select className="form-control" name={fieldPropsFilter.name} onChange={fieldPropsFilter.onChange} value={String(fieldPropsFilter.value)} style={{ borderColor: (errors.filter && touched.filter) ? "red" : "", borderWidth: (errors.filter && touched.filter) ? "2px" : "" }} onBlur={fieldPropsFilter.onBlur}>
              <option>-</option>
